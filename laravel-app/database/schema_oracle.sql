...
CREATE SEQUENCE pets_seq START WITH 1 INCREMENT BY 1 NOCACHE;
CREATE SEQUENCE adoption_requests_seq START WITH 1 INCREMENT BY 1 NOCACHE;
CREATE SEQUENCE veterinary_appointments_seq START WITH 1 INCREMENT BY 1 NOCACHE;
CREATE SEQUENCE medical_records_seq START WITH 1 INCREMENT BY 1 NOCACHE;
...
CREATE OR REPLACE TRIGGER pets_bir
BEFORE INSERT ON pets
FOR EACH ROW
BEGIN
    IF :NEW.pet_id IS NULL THEN
        SELECT pets_seq.NEXTVAL INTO :NEW.pet_id FROM dual;
    END IF;
END;
/

CREATE OR REPLACE TRIGGER adoption_requests_bir
BEFORE INSERT ON adoption_requests
FOR EACH ROW
BEGIN
    IF :NEW.request_id IS NULL THEN
        SELECT adoption_requests_seq.NEXTVAL INTO :NEW.request_id FROM dual;
    END IF;
END;
/

CREATE OR REPLACE TRIGGER veterinary_appointments_bir
BEFORE INSERT ON veterinary_appointments
FOR EACH ROW
BEGIN
    IF :NEW.appointment_id IS NULL THEN
        SELECT veterinary_appointments_seq.NEXTVAL INTO :NEW.appointment_id FROM dual;
    END IF;
END;
/
...
BEGIN
    IF :NEW.record_id IS NULL THEN
        SELECT medical_records_seq.NEXTVAL INTO :NEW.record_id FROM dual;
    END IF;
END;
/
